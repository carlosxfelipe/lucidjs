...
export type Setter<T> = undefined extends T ? <U extends T>(
    value?: (U extends Function ? never : U) | ((prev?: T) => U),
  ) => U
  : <U extends T>(
    value: (U extends Function ? never : U) | ((prev: T) => U),
  ) => U;
...
export function createSignal<T>(
  ...
  get: () => T,
  set: (v: (T extends Function ? never : T) | ((prev: T) => T)) => T,
] {
  return [
    () => value as T,
    (v) => {
      return (value = typeof v === "function"
        ? (v as (prev: T) => T)(value)
        : v);
    },
  ];
...
export function createSelector<T>(
  source: () => T,
  fn: (k: T, value: T) => boolean = equalFn,
) {
