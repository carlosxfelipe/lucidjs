...
function getContextId(count: number) {
  const num = String(count),
    len = num.length - 1;
  return sharedConfig.context!.id + (len ? String.fromCharCode(96 + len) : "") +
    num;
}
...
export function createResource<T, S>(
  ...
  let value = options.storage
    ? options.storage(options.initialValue)[0]()
    : options.initialValue;
  let p: Promise<T> | T | null | undefined;
  let error: any;
  if (sharedConfig.context!.async && options.ssrLoadFrom !== "initial") {
    resource = sharedConfig.context!.resources[id] ||
      (sharedConfig.context!.resources[id] = {});
    if (resource.ref) {
      if (
        !resource.data && !resource.ref[0].loading && !resource.ref[0].error
      ) {
  ...
  const read = () => {
    ...
    const resolved = options.ssrLoadFrom !== "initial" &&
  ...
  function load() {
    ...
    if (!ctx.async) {
      return (read.loading =
        !!(typeof source === "function" ? (source as () => S)() : source));
    }
...
export function Suspense(props: { fallback?: string; children: string }) {
  ...
  const id = sharedConfig.getContextId();
  const o = createOwner();
  const value: SuspenseContextType = ctx.suspense[id] ||
    (ctx.suspense[id] = {
