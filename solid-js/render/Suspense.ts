...
type SuspenseListContextType = {
  ...
  ) => Accessor<SuspenseListRegisteredState>;
};

type SuspenseListRegisteredState = {
  showContent: boolean;
...
const suspenseListEquals = (
  a: SuspenseListRegisteredState,
  b: SuspenseListRegisteredState,
) => a.showContent === b.showContent && a.showFallback === b.showFallback;
const SuspenseListContext = /* #__PURE__ */ createContext<
  SuspenseListContextType
>();
    ...
    (prev: Partial<SuspenseListState>) => {
      ...
      for (let i = 0, len = reg.length; i < len; i++) {
        ...
        if (!stop && !s) {
          ...
          res[n] = {
            showContent: next,
            showFallback: !tail || (next && tail === "collapsed")
              ? showFallback
              : false,
          };
          stop = true;
