 ...
 */
export type FlowProps<P extends Record<string, any> = {}, C = JSX.Element> =
  & P
  & { children: C };
/**
 ...
 * Note that all JSX <Elements> are of the type `JSX.Element`.
 */
export type FlowComponent<P extends Record<string, any> = {}, C = JSX.Element> =
  Component<
    FlowProps<P, C>
  >;

/** @deprecated: use `ParentProps` instead */
export type PropsWithChildren<P extends Record<string, any> = {}> = ParentProps<
  P
>;
...
type Override<T, U> = T extends any ? U extends any ?
      & {
        [K in keyof T]: K extends keyof U ? DistributeOverride<U[K], T[K]>
          : T[K];
      }
      & {
        [K in keyof U]: K extends keyof T ? DistributeOverride<U[K], T[K]>
          : U[K];
      }
  : T & U
  : T & U;
...
type _MergeProps<T extends unknown[], Curr = {}> = T extends [
  ...
  : T extends [] ? Curr
  : T extends (infer I | (() => infer I))[] ? OverrideSpread<Curr, I>
  : Curr;
...
export function mergeProps<T extends unknown[]>(...sources: T): MergeProps<T> {
  ...
  const defined: Record<string, PropertyDescriptor | undefined> = Object.create(
    null,
  );
...
>(props: T, ...keys: K): SplitProps<T, K> {
  ...
  for (const propName of Object.getOwnPropertyNames(props)) {
    ...
    const isDefaultDesc = !desc.get && !desc.set && desc.enumerable &&
